...
@section('title', 'Criar Instância WhatsApp - Painel Administrativo')
@section('page-title', 'Criar Nova Instância WhatsApp')
@section('page-subtitle', 'Configure uma nova instância WhatsApp')
        ...
        <form action="{{ route('admin.whatsapp.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                >
                    ...
                    <option value="connected" {{ old('status') == 'connected' ? 'selected' : '' }}>Conectado</option>
                    <option value="disconnected" {{ old('status') == 'disconnected' ? 'selected' : '' }}>Desconectado</option>
                    <option value="error" {{ old('status') == 'error' ? 'selected' : '' }}>Erro</option>
                </select>
                @error('status')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200">
                <button type="submit" class="btn-primary">
                    <i class="fas fa-save mr-2"></i> Criar Instância
                </button>
