...
<head>
    ...
    <link rel="stylesheet" href="{{ asset(mix('panel_assets/css/app.css')) }}">
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/shepherd.js/7.1.2/js/shepherd.js" integrity="sha512-c7PZ7h4N0huwVjMpvqO3PNTM1r96IXQMAJv78+ciej3E1LUrW1yb97XO5gag5LsdXiFKOr9eGiz07GTNejCWRA==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
</head>
<body>
...
<div id="app">
    ...
    <div id="main">
                ...
                <ul class="navbar-nav d-flex align-items-center navbar-light ml-auto">
                    <li class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item" href="{{ route('panel.users.edit') }}">
                                <i data-feather="user"></i>
                                Perfil
                            </a>
                            <div class="dropdown-divider"></div>
                            <form action="{{ route('panel.auth.logout') }}" class="dropdown-item" method="post">
                                @csrf
        ...
        <div class="main-content container-fluid">
            @if(Route::currentRouteName() != 'panel.configPreviews.create')
                <div class="alert alert-primary" role="alert">
                    Acesse sua loja
                    <a style="text-decoration: underline;" target="_blank"
                       href="{{ route('web.systemSettings.template', ['domain' => Auth::user()->domain]) }}">{{ route('web.systemSettings.template', ['domain' => Auth::user()->domain]) }}</a>
                    para visualizar suas alterações
                </div>

                <div class="page-title">
                    <h3>@yield('title')</h3>
                    <p class="text-subtitle text-muted">Este é a area de
                        <span class="text-lowercase">@yield('title')</span>
                    </p>
                </div>
            @endif
            @yield('content')
...
@if(Auth::user()->popup_show)
    <script>
        $('#alertMessageModal').modal('show');

        $.post("{{ route('panel.users.disablePopup') }}");
    </script>
@endif
@stack('scripts')
